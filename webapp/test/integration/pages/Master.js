			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("Belnr") === oObject.Belnr;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Belnr;
									} else {
										sCurrentId = aItemsNotInTheList[0].Belnr;
									}
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Belnr"),
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function() {
					return this.waitFor({
						...
						success: function() {
							Opa5.assert.ok(true, "The list header displays 'ALVSet (0)'");
						},
				...
				theListShouldHaveAllEntries: function() {
					...
					// retrieve all ALVSet to be able to check for the total amount
